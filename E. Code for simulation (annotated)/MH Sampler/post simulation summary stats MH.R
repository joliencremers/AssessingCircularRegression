SumStat_l<-function(nsim,n,B11,B12,a1,a2,mu,sd,tm,burn,t.lag,seed=101,date,no, BURN=0, fileloc){
  ...
  load(file=paste(fileloc, "results", no, ".Rdata", sep=""))
  ...
  relbias[] <-  abs(c(a1, B11, a2, B12))/abs(bias[]) 
  MSE[]	     <-    bias[]^2+SD[]^2
  ...
  write.table(results, paste(fileloc,"resultssimulationSUMMARY",date, "_#", no, ".txt", sep=""), sep="\t")
  write.table(parameters, paste(fileloc,"resultssimulationSUMMARY",date, "_#", no, ".txt", sep=""), sep="\t", append=T)
  
  #Create a simple latex table for the results
  print(xtable(results), type="latex", file=paste(fileloc,"resultssimulationLATEX",date, "_#", no, ".tex", sep=""))
...
SumStat_2l<-function(nsim,n,B11,B12,B21,B22,a1,a2,mu,sd,dep,tm,burn,t.lag, seed=101,date,no,BURN=0, fileloc){
  ...
  hi95HPD[]	   <-    apply(cbind(sum[,7,]),1,mean)
  bias[]	   <-    apply(cbind(sumres[,1,]),1,mean)
  relbias[] <-   abs(c(a1, B11, B12, a2, B21, B22))/abs(bias[]) 
  MSE[]	     <-	   bias[]^2+SD[]^2
  coverage[] <-    apply(cbind(sumres[,2,]),1,mean)*100
  coverageHPD[] <-    apply(cbind(sumres[,4,]),1,mean)*100
  aiw[]	     <-    apply(cbind(sumres[,3,]),1,mean)
...
SumStat_1c<-function(nsim,n,B11,B12,B21,B22,a1,a2,mu_c,k,tm,burn,t.lag, seed=101,date,no,BURN=0, fileloc){
...
SumStat_l2<-function(nsim,n,B11,B12,a1,a2,mu,sd,tm,burn,t.lag,seed=101,date,no, BURN=0, fileloc){
...
SumStat_2l2<-function(nsim,n,B11,B12,B21,B22,a1,a2,mu,sd,dep,tm,burn,t.lag, seed=101,date,no,BURN=0, fileloc){
...
SumStat_1c2<-function(nsim,n,B11,B12,B21,B22,a1,a2,mu_c,k,tm,burn,t.lag, seed=101,date,no,BURN=0, fileloc){
