...
Simulation_l<-function(nsim,n,B11,B12,a1,a2,mu,sd,tm,burn,t.lag,seed=101,date,no,BURN=0, data){
  ...
  res		   <-	array(NA, dim=c((tm), (4), nsim), dimnames=list(NULL, c("B1a", "B1b", "B2a", "B2b"), NULL))
  time	   <-	array(NA, dim=c(nsim))
  sumres	 <-	array(NA, dim=c((4),5,nsim), dimnames=list(c("B1a", "B1b", "B2a", "B2b"), c("Bias", "Coverage", "Interval width", "CoverageHPD", "aiwHPD")))
  ...
  for (i in 1:nsim){
    ...
    start.time	<-	Sys.time()
    #Run sampler
    res[ , ,i]	<-	CircReg(theta~x1, data=as.data.frame(data[,,i]) , tm=tm, burn=burn, t.lag=t.lag)
    #Save end time
    end.time	<-	Sys.time()
    #Compute time taken to run the sampler on 1 dataset
  ...
  save(res, file=paste("results",no,date,".Rdata", sep=""))
  ...
  results		      <-	cbind(mean, median, mode, lo95, lo95HPD, hi95,hi95HPD, SD, MSE, bias, coverage, coverageHPD, aiw, aiwHPD, MeanCompT)
  
  #Write the means of the summary statistics (and the metadata; input to simulation function) to a file
  write.table(results, paste("resultssimulationSUMMARY",date, "_#", no, ".txt", sep=""), sep="\t")
  write.table(parameters, paste("resultssimulationSUMMARY",date, "_#", no, ".txt", sep=""), sep="\t", append=T)
 
  #Create a simple latex table for the results
  print(xtable(results), type="latex", file=paste("resultssimulationLATEX",date, "_#", no, ".tex", sep=""))
...
Simulation_2l<-function(nsim,n,B11,B12,B21,B22,a1,a2,mu,sd,dep,tm,burn,t.lag, seed=101,date,no,BURN=0, data){
  ...
  res		   <-	array(NA, dim=c((tm), (6), nsim), dimnames=list(NULL, c("B1a", "B1b1", "B1b2", "B2a", "B2b1", "B2b2"), NULL))
  time	   <-	array(NA, dim=c(nsim))
  sumres	 <-	array(NA, dim=c((6),5,nsim), dimnames=list(c("B1a", "B1b1", "B1b2", "B2a", "B2b1", "B2b2"), c("Bias", "Coverage", "Interval width", "CoverageHPD", "aiwHPD")))
  ...
  for (i in 1:nsim){
    ...
    start.time	<-	Sys.time()
    #Run sampler
    res[ , ,i]	<-	CircReg(theta~x1+x2, data=as.data.frame(data[ , ,i]), tm=tm, burn=burn, t.lag=t.lag)
    #Save end time
    end.time	  <-	Sys.time()
    #Compute time taken to run the sampler on 1 dataset
    time.taken 	<-	difftime(end.time,start.time,units="secs")
...
Simulation_1c<-function(nsim,n,B11,B12,B21,B22,a1,a2,mu_c,k,tm,burn,t.lag, seed=101,date,no,BURN=0, data){
...
#1 circular and 1 linear predictor

Simulation_1c1l<-function(nsim,n,B11,B12,B13,B21,B22,B23,a1,a2,mu_c,k,mu,sd,tm,burn,t.lag, seed=101,date,no,BURN=0, data){
  ...
  #Create empty arrays
  simdata   <-    array(NA, dim=c(n, 4, nsim), dimnames=list(NULL, c("theta", "x1", "x2", "x3"), NULL))
  res       <-	  array(NA, dim=c((tm), (8), nsim), dimnames=list(NULL, c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"), NULL))
  time	    <-	  array(NA, dim=c(nsim))
  sumres	  <-	  array(NA, dim=c((8),5,nsim), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"), c("Bias", "Coverage", "Interval width", "CoverageHPD", "aiwHPD")))
  mean	    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("Mean")))
  mode      <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("Mode")))
  median	  <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("Median")))
  SD		    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("SD")))
  MSE		    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("MSE")))
  lo95	    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("lower bound 95% CI")))
  hi95	    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("upper bound 95% CI")))
  lo95HPD      <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("lower bound 95% HPD")))
  hi95HPD	    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("upper bound 95% HPD")))
  bias	    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("Bias")))
  coverage	<-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("Coverage")))
  coverageHPD  <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("CoverageHPD")))
  aiw		    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("aiw")))
  aiwHPD  	    <-    array(NA, dim=c((8),1), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"),c("aiwHPD")))
  sum		    <-	  array(NA, dim=c((8),8,nsim), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B3b3"), c("mean", "lo95", "hi95", "SD", "Median", "lo95HPD", "hi95HPD", "Mode"), NULL))
  ...
  for (i in 1:nsim){
    ...
    start.time	<-	Sys.time()
    #Run sampler
    res[ , ,i]	<-	CircReg(theta~x1+x2+x3, data=as.data.frame(data[ , ,i]), tm=tm, burn=burn, t.lag=t.lag)
    #Save end time
    end.time	  <-	Sys.time()
    #Compute time taken to run the sampler on 1 dataset
    time.taken  <-	difftime(end.time,start.time,units="secs")
  ...
  #Create and empty array for a result summary table and give names to the columns
  parameters	    <-	array(0, dim=c(1,21), dimnames=list(c("metadata"), c("nsim", "n", "a1", "a2", "B11", "B12", "B13", "B21", "B22", "B23", "tm", "burn", "t.lag", "seed", "mu_c", "k", "mu","sd", "date", "no", "BURN")))
  parameters[1, ]	<-	c(nsim,n, a1,a2,B11,B12,B13,B21,B22,B23,tm,burn,t.lag, seed, mu_c, k, mu,sd,date,no,BURN)
...
Simulation_diff<-function(nsim,n,B11,B12,B13,B21,B22,B23, B24,a1,a2,mu_c,k,mu1,sd1,mu2,sd2,tm,burn,t.lag, seed=101,date,no,BURN=0, data){
  ...
  res       <-    array(NA, dim=c((tm), (9), nsim), dimnames=list(NULL, c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B2b3", "B2b4"), NULL))
  time	    <-	  array(NA, dim=c(nsim))
  sumres	  <-	  array(NA, dim=c((9),5,nsim), dimnames=list(c("B1a", "B1b1", "B1b2", "B1b3", "B2a", "B2b1", "B2b2", "B2b3", "B2b4"), c("Bias", "Coverage", "Interval width", "CoverageHPD", "aiwHPD")))
  ...
  for (i in 1:nsim){
    ...
    start.time	<-	Sys.time()
    #Run sampler
    res[ , ,i]	<-	CircReg(theta~x1+x2+x3, theta~x1+x2+x3+x4, data=as.data.frame(data[ , ,i]), tm=tm, burn=burn, t.lag=t.lag)
    #Save end time
    end.time	  <-	Sys.time()
    #Compute time taken to run the sampler on 1 dataset
    time.taken  <-	difftime(end.time,start.time,units="secs")
